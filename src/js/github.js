...
// Written in ES5-compatible syntax for Vite 0.10.x / old terser
...
function getTopics(repo) {
  ...
  return repo.repositoryTopics.nodes.map(function(n) {
    return n.topic.name;
  }).slice(0, 4);
}
...
function buildTopicsHtml(topics) {
  ...
  var inner = topics.map(function(t) {
    return '<span class="gh-topic">' + t + '</span>';
  }).join('');
  return '<div class="gh-topics">' + inner + '</div>';
}

function buildLiveLinkHtml(url) {
  if (!url) return '';
  return '<a href="' + url + '" class="gh-btn gh-btn-live" target="_blank" rel="noopener">' +
...
function buildLangHtml(lang) {
  if (!lang) return '';
  return '<span class="gh-lang">' +
    '<span class="gh-lang-dot" style="background:' + langColor(lang) + '"></span>' +
    lang.name +
    '</span>';
}
...
function buildCard(repo, index) {
  ...
  var topicsHtml = buildTopicsHtml(topics);
  var liveLink = buildLiveLinkHtml(repo.homepageUrl);
  var langHtml = buildLangHtml(repo.primaryLanguage);
...
function normaliseRestRepos(repos) {
  return repos
    .filter(function(r) { return !r.fork && !r.private; })
    .sort(function(a, b) { return b.stargazers_count - a.stargazers_count; })
    .slice(0, 6)
    .map(function(r) {
      var topics = (r.topics || []).map(function(t) { return { topic: { name: t } }; });
      var lang = r.language ? { name: r.language, color: null } : null;
      var license = r.license ? { name: r.license.name } : null;
      return {
        name: r.name,
...
function fetchRestRepos() {
  return fetch('https://api.github.com/users/' + GITHUB_USERNAME + '/repos?sort=updated&per_page=30')
    .then(function(res) {
      if (!res.ok) throw new Error('GitHub REST API unavailable');
      return res.json();
    })
    .then(function(repos) {
      return normaliseRestRepos(repos);
    });
}

function fetchPinnedRepos() {
  var query = '{ user(login: "' + GITHUB_USERNAME + '") { pinnedItems(first: 6, types: REPOSITORY) { nodes { ... on Repository { name description url homepageUrl stargazerCount forkCount primaryLanguage { name color } repositoryTopics(first: 6) { nodes { topic { name } } } updatedAt openGraphImageUrl isPrivate licenseInfo { name } } } } } }';
...
export function initGitHubProjects() {
  ...
  fetchPinnedRepos()
    .then(function(repos) {
      ...
      if (statusEl) {
        var label = repos.length === 1 ? 'project' : 'projects';
        statusEl.textContent = repos.length + ' ' + label + ' · auto-synced from GitHub';
      }

      container.innerHTML = repos.map(function(r, i) {
